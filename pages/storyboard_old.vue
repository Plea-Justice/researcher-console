  ...
  <section class="section">
    <div class="container">
      ...
      <StoryFrame
        ...
        :key="`frame_${index}`"
        v-on:move-up="moveUp(index, ...arguments)"
        :frame="frame"
...
export default {
  ...
  computed: {
    scenes() {
      return this.$store.state.scenes.sceneGrid;
    }
  },
  methods: {
    expand() {
    ...
    moveUp(frameIndex, sceneIndex, e) {
      console.log(
        "Moving Up { Frame: " + frameIndex,
        ", Scene: " + sceneIndex + ", event: " + e + " } "
      );
    }
  },
  head() {
